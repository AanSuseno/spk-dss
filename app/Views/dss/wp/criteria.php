                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td class="tippy-me" data-tippy-content="<?= $c['weight'] ?>/<?= $total_criteria_weight ?>"><?= number_format(($c['weight']/$total_criteria_weight), 2, ".", ",") ?></td>
                                            <td>
                                                <button
...
<div class="modal fade" id="modalCreateCriteria">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="<?= base_url('wp/' . $id_project . '/criteria/create') ?>" method="post">
...
<script>
    function deleteCriteriaModal(id) {
        $('#form-delete').attr('action', '<?= base_url("wp/$id_project/criteria/delete") ?>/'+id)
    }

    function subCriteriaModal(id_criteria, criteria_name) {
        $('#modalSubCriteria').modal()
        $('#criteria-name').text(criteria_name)
        $('#form-create-sub-criteria').attr('action', '<?= base_url("wp/$id_project/sub_criteria/create/") ?>'+id_criteria)

        $.ajax({
            url: '<?= base_url("wp/$id_project/sub_criteria/") ?>'+id_criteria, // Specify the URL of the API or JSON file
                ...
                sub_criteria.forEach((i, idx) => {
                    str_table += `
                        <tr>
                            ...
                            <td>${i.weight}</td>
                            <td>
                                <a href="<?= base_url("wp/$id_project/sub_criteria/delete/") ?>${i.id}" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                    `
                })
